...
{margin: 0px; font-weight: bold; font-family: <?php echo stripslashes($wpcs_settings['font_family']); ?>;
text-decoration:none;font-size: <?php echo $wpcs_settings['heading_font']; ?>px; color:<?php echo $wpcs_settings['heading_color']; ?>;}
#wpcontent_slider .cs-title {width: 880px; padding: 10px 40px; background: <?php echo $wpcs_settings['background_color']; ?>; color: <?php echo $wpcs_settings['color']; ?>; font-family: <?php echo stripslashes($wpcs_settings['font_family']); ?>; font-size: <?php echo $wpcs_settings['font_size']; ?>px; letter-spacing: normal;line-height: normal;}
#wpcontent_slider_container .cs-prev,#wpcontent_slider_container .cs-next {font-weight: bold;
...
.cs-prev, .cs-next{width: 38px; height: 46px; display: block;}

.cs-prev{background: url(../../asis_images/prev_arrow.png) no-repeat center center;}
.cs-next{background: url(../../asis_images/next_arrow.png) no-repeat center center;}
#cs-wpcontent_slider11{background-repeat: no-repeat}
...
function wp_content_slider()
{
	...
	if($wpcs_settings['order']=='true')
	{
		...
		<?php if($images['heading'] || $images['text']){ ?>
		
		
		
        <span class="content-slide-caption"><?php if($images['heading']){ echo '<h4>'.$images['heading'].'</h4>';} ?><?php if($images['text']){ echo $images['text'];} ?></span>
